...
To generate the required system ROMs:
1. Obtain the `apple2gs.zip` MAME ROM package.
...
make
```
This generates the boot ROMs:
* **ROM Version 3**: `boot.rom`
* **ROM Version 1**: `boot1.rom`

Place `boot.rom` and/or `boot1.rom` on the root of your MiSTer SD card or in `/media/fat/games/Apple-IIgs/`. You can switch between ROM versions dynamically using the MiSTer OSD menu.

## Development & Simulation

This project supports both Verilator and vsim simulations.

For details on building and running simulation test benches:
- Refer to CLAUDE.md for build commands, simulation run parameters, keyboard/mouse input injection, and debugging macros.
- Refer to regression test results and plans under the `doc/` directory.
