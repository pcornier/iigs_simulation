...
void DumpInstruction() {
	...
	case 0xC7: sta = "cmp"; type = direct24; opType = byte2; break;
	...
	//case 0xA2: sta = "ldx"; type = immediate; opType = byte2; break;
	case 0xA2: sta = "ldx"; type = immediate; opType = tool;  break;
	...
	case 0x22: sta = "jsl"; type = longValue; opType = longOp; break;
	...
	if (ins_index > 1) {
		...
		if (opType == tool) {
			...
			{
				if (gs_tools[item].addr == operand)
				{
					ins_str[1] = "_";
					ins_str[1].append(gs_tools[item].name);
					ins_str[1].append(fmt::format(" ({0:02x}{1:02x})", ins_in[2], ins_in[1]));
					type = instruction_type::replace;
					break;
				}
				item++;
			}
		...
		if (opType == byte3 || opType == longOp) {
			...
			if (d == 0 || opType==longOp) {
				...
				//	console.AddLog(fmt::format("{0} {1:02x}", i, ins_in[i]).c_str());
				//}
	...
	case instruction_type::replace: sta = "";  arg1 = ins_str[1]; f = "{2:s}"; break;
	case immediate:
		if (ins_index == 3) {
		...
		if (clk_sys.clk != clk_sys.old) {
			...
			if (cpu_clock != cpu_clock_last && cpu_reset == 0) {
				...
				if (en) {
					...
					if (vpa && nextstate == 1) {
						if (ins_index > 0) {
							//console.AddLog(fmt::format("INS: PC={0:06x} IN={1:02x} MA={2:06x}", ins_pc[0], ins_in[0], ins_ma[0]).c_str());
							DumpInstruction();
						}
