...
<style>
  ...
  .badge.platform-iigs    { background: #06a; }
  .badge.platform-apple2  { background: #850; }
  .badge.platform-unknown { background: #666; font-weight: normal; }
  .count.iigs    { background: #06a; }
  .count.apple2  { background: #850; }
  .count.unknown { background: #666; }
...
<div class="toolbar">
  ...
  <div class="filter-row">
    ...
    <button class="platform-filter" data-filter="iigs">IIgs</button>
    <button class="platform-filter" data-filter="apple2">Apple II</button>
    <button class="platform-filter" data-filter="unknown">Unknown</button>
    <span class="summary" id="platformSummary"></span>
  </div>
  <div class="filter-row">
    <label>Triage:</label>
  ...
  <div class="actions">
    ...
    <button id="exportVisibleBtn" title="Download the paths currently shown by filters">⬇ Export visible list</button>
  </div>
</div>
...
#
# If woz_report/catalog.tsv exists (generated by woz_classify_platform.py),
# each row also gets a data-platform="iigs|apple2|unknown" attribute so the
# UI can filter IIgs titles separately from the Apple II-era stuff.
# Tab-separated because paths can contain commas and quotes, but never tabs.
...
awk -F',' -v catalog="$CATALOG" '
  ...
  BEGIN {
    # Load the catalog first (if present) into platform_of[path].
    # Catalog is TSV: woz_path<TAB>platform<TAB>disk_type<TAB>note
    if ((getline line < catalog) > 0) {
      # skip header
      while ((getline line < catalog) > 0) {
        n = split(line, cols, "\t")
        if (n < 2) continue
        platform_of[cols[1]] = cols[2]
      }
      close(catalog)
    }
  }
  NR > 1 {
    ...
    hash   = $3
    # rc = $5
    # woz_path is field 6+. Reconstruct across embedded commas (filenames
    # like "Fractions (addition, subtraction) 800K.woz" span $6..$N).
    path = $6
    ...
    # Lookup platform (defaults to "unknown" if catalog missing or stale)
    ...
    rows[++nrows] = sort_key SUBSEP status SUBSEP size SUBSEP hash SUBSEP path SUBSEP fname SUBSEP platform
  }
  END {
    # sort by sort_key
    ...
    for (i = 1; i <= nrows; i++) {
      ...
      printf "<div class=\"row\" data-path=\"%s\" data-status=\"%s\" data-hash=\"%s\" data-platform=\"%s\">\n", pattr, status, hash, platform
      if (hash != "") {
      ...
      printf "      <span class=\"badge platform-%s\">%s</span>\n", platform, platform
...
(function() {
  ...
  function currentPlatformFilter() {
    const b = document.querySelector('.platform-filter.active');
    return b ? b.dataset.filter : 'all';
  }
  function applyFilters() {
    ...
    document.querySelectorAll('.row').forEach(row => {
      ...
      const p = row.dataset.platform || 'unknown';
      const sOk = sf === 'all' || s === sf;
      const tOk = tf === 'all' ||
                  (tf === 'unreviewed' && t === 'unreviewed') ||
                  t === tf;
      const pOk = pf === 'all' || p === pf;
      const show = sOk && tOk && pOk;
  ...
  function updateSummary(map) {
    ...
    paint('platformSummary', platformCounts, ['iigs','apple2','unknown']);
  }
  ...
  document.querySelectorAll('.platform-filter').forEach(b => {
    b.addEventListener('click', () => {
      document.querySelectorAll('.platform-filter').forEach(x => x.classList.remove('active'));
      b.classList.add('active');
      applyFilters();
    });
  });
  ...
  document.getElementById('exportVisibleBtn').addEventListener('click', () => {
    const paths = [];
    document.querySelectorAll('.row:not(.hidden)').forEach(r => paths.push(r.dataset.path));
    if (!paths.length) { alert('Nothing visible'); return; }
    const blob = new Blob([paths.join('\n') + '\n'], {type: 'text/plain'});
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = 'retest-visible.txt';
    a.click();
  });
